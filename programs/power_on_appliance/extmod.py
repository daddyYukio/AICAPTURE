...
async def main():
    ...
    # 起動時にSwitchBotデバイスをスキャン
    switchbot = await SwitchBot.auto_detect()

    if switchbot == None:
        print(f"No switchbot found!!")
    else:
        # 見つかったら電源OFFにしてから開始
        await switchbot.turn_off()
    ...
    while True:
        ...
        try:
            ...
            if len(res) > 0:
                ...
                # 電源ON
                # POWER_ON_TIME_SEC経過すると自動で電源OFF
                # 自動で電源OFF前に再度コールすると、電源OFFが延長される
                if switchbot is not None:
                    switchbot.switch_on_with_timer(POWER_ON_TIME_SEC)
