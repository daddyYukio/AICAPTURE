...
def start_recording():
    """
    録画を開始する
    REC_DURATION_SEC後に自動的に録画を停止する
    録画中にコールされた場合は、その時点からREC_DURATION_SECまで録画が延長される
    """
    global _rec_timer
    ...
    if _rec_timer is not None:
        _rec_timer.cancel()
        print("Resetting recording timer")
    else:
        aicap.start_rec()
        print("Start recording")

    _rec_timer = threading.Timer(REC_DURATION_SEC, stop_recording)
    _rec_timer.start()
